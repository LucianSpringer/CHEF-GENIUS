...
function App() {
  ...
  const [ingredients, setIngredients] = useState<string[]>([]); // Raw detected ingredients
  ...
  const [timer, setTimer] = useState<{ secondsLeft: number, isActive: boolean, duration: number } | null>(null);
        ...
        recognition.onresult = (event: any) => {
          ...
          if (command.includes('next')) {
             ...
             handlePrevStep();
              ...
              if (recipe) {
                  const currentInst = recipe.instructions[cookingStep];
  ...
  }, [appState, voiceEnabled, cookingStep, recipe]); 
  ...
  const saveRecipe = () => {
    ...
    const finalSourceUrl = manualSourceUrl.trim() ? manualSourceUrl.trim() : recipe.sourceUrl;
  ...
  const handleGenerateRecipe = async () => {
    if (ingredients.length === 0 && userProfile.customIngredients.length === 0) return;
    ...
    try {
      const [recipeResult, priceResult] = await Promise.all([
        generateRecipe(ingredients, userProfile),
        fetchIngredientPrices(ingredients)
      ]);
      ...
      console.error(err);
      setError("Failed to generate recipe. Please check your connection.");
      setAppState(AppState.INGREDIENT_CONFIRMATION);
    } finally {
      setLoading(false);
    }
  ...
  const handleGenerateShoppingList = async () => {
      ...
      try {
          ...
          const list = await generateShoppingList(ingredientNames, userProfile.pantryStaples);
  ...
  const handleGetSubstitution = async (ingredientName: string) => {
      ...
      try {
          const sub = await getSubstitutions(ingredientName, recipe.title);
  ...
  const addIngredient = (val: string) => {
    if (val.trim()) setIngredients(prev => [...prev, val.trim()]);
  };
  ...
  const addCustomIngredient = (val: string) => {
      if (val.trim() && !userProfile.customIngredients.includes(val.trim())) {
          updateProfile({
              ...userProfile,
              customIngredients: [...userProfile.customIngredients, val.trim()]
          });
      }
  };
  const removeCustomIngredient = (idx: number) => {
      updateProfile({
          ...userProfile,
          customIngredients: userProfile.customIngredients.filter((_, i) => i !== idx)
      });
  };

  const addPantryStaple = (val: string) => {
      if (val.trim() && !userProfile.pantryStaples.includes(val.trim())) {
          updateProfile({
              ...userProfile,
              pantryStaples: [...userProfile.pantryStaples, val.trim()]
          });
      }
  };
  ...
  const removePantryStaple = (idx: number) => {
      updateProfile({
          ...userProfile,
          pantryStaples: userProfile.pantryStaples.filter((_, i) => i !== idx)
      });
  };

  // Cooking Mode Navigation & Timer
  
  const handleNextStep = () => {
      setCookingStep(prev => {
          if (recipe && prev < recipe.instructions.length - 1) {
              const nextStep = prev + 1;
              setTimer(null); // Reset timer on step change
              return nextStep;
          }
          return prev;
      });
  ...
  const startTimer = (seconds: number) => {
      setTimer({ secondsLeft: seconds, duration: seconds, isActive: true });
  };
    ...
    <header className="bg-white border-b border-slate-200 sticky top-0 z-50">
      <div className="max-w-6xl mx-auto px-4 h-16 flex items-center justify-between">
        ...
        >
          ...
          <h1 className="text-xl font-bold tracking-tight text-slate-900">ChefGenius</h1>
        </div>
        ...
        <div className="bg-white p-8 rounded-2xl shadow-sm border border-slate-200 space-y-8">
          ...
          <div>
            ...
            <div className="flex flex-wrap gap-3">
              {DIETARY_OPTIONS.map(opt => (
                <button
                  key={opt}
                  onClick={() => updateProfile({ ...userProfile, dietaryRestrictions: toggleSelection(userProfile.dietaryRestrictions, opt) })}
                  className={`px-4 py-2 rounded-full text-sm font-medium border transition-all ${
                    userProfile.dietaryRestrictions.includes(opt) 
          ...
          <div>
             ...
             <div className="flex flex-wrap gap-3">
              {ALLERGY_OPTIONS.map(opt => (
                <button
                  key={opt}
                  onClick={() => updateProfile({ ...userProfile, allergies: toggleSelection(userProfile.allergies, opt) })}
                  className={`px-4 py-2 rounded-full text-sm font-medium border transition-all ${
          ...
          <div>
            ...
            <input
              ...
              value={userProfile.cuisinePreferences.join(", ")}
              onChange={(e) => updateProfile({ ...userProfile, cuisinePreferences: e.target.value.split(',').map(s => s.trim()) })}
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              ...
              <div>
                ...
                <div className="flex flex-wrap gap-2">
                    {userProfile.customIngredients.map((ing, idx) => (
              ...
              <div>
                ...
                <div className="flex flex-wrap gap-2">
                    {userProfile.pantryStaples.map((ing, idx) => (
  ...
  const renderSavedRecipes = () => {
    ...
    if (matchProfile) {
        if (userProfile.allergies.length > 0) {
             filtered = filtered.filter(r => {
                 // Handle potential string vs object ingredients for safety
                 const ingString = r.ingredients.map(i => typeof i === 'string' ? i : i.name).join(' ');
                 const allText = (ingString + r.title).toLowerCase();
                 return !userProfile.allergies.some(allergen => allText.includes(allergen.toLowerCase()));
             });
        }
        if (userProfile.dietaryRestrictions.length > 0) {
            filtered = [...filtered].sort((a, b) => {
                const aScore = userProfile.dietaryRestrictions.reduce((acc, res) => acc + ((a.title + a.description).includes(res) ? 1 : 0), 0);
                const bScore = userProfile.dietaryRestrictions.reduce((acc, res) => acc + ((b.title + b.description).includes(res) ? 1 : 0), 0);
  ...
  const renderIdle = () => (
    <div className="flex flex-col items-center justify-center min-h-[80vh] px-4 text-center">
      ...
      <p className="text-slate-500 max-w-md mb-8 text-lg">
        Upload a photo of your ingredients. Our Nano Banana vision model will analyze them, suggest a recipe, check live prices, and help you cook.
      </p>
      ...
      <div className="mt-6 flex flex-col items-center gap-2">
        ...
        {userProfile.customIngredients.length > 0 && (
             <div className="flex gap-2 items-center text-sm text-slate-500">
                <span>Including:</span>
                {userProfile.customIngredients.map(r => (
  ...
  const renderIngredientConfirmation = () => (
    <div className="max-w-2xl mx-auto mt-12 px-4">
      <h2 className="text-2xl font-bold text-slate-900 mb-2">Confirm Ingredients</h2>
      ...
      <div className="flex justify-end gap-4">
        ...
        <button 
          ...
          disabled={ingredients.length === 0 && userProfile.customIngredients.length === 0}
        ...
        >
          {loading ? <Spinner /> : 'Generate Recipe'}
        </button>
      </div>
    </div>
  );
      ...
      <div className="max-w-4xl mx-auto px-4 py-8 pb-24">
        ...
        <div className="flex justify-between items-start mb-6">
           <div className="flex items-center gap-2">
                <span className="px-2 py-1 bg-orange-100 text-orange-700 rounded text-xs font-bold uppercase tracking-wide">{recipe.cuisine}</span>
                <span className="px-2 py-1 bg-blue-100 text-blue-700 rounded text-xs font-bold uppercase tracking-wide">{recipe.nutrition.calories} kcal</span>
            </div>
                ...
                <div className="relative">
                    <button 
                        onClick={() => {
                            ...
                            if (!manualSourceUrl && recipe.sourceUrl) setManualSourceUrl(recipe.sourceUrl);
                        }}
                        className="flex items-center gap-2 px-4 py-2 bg-white border border-slate-200 text-slate-700 font-medium rounded-lg hover:bg-slate-50 transition-colors shadow-sm"
                    >
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-8 mb-12">
          <div>
            <h1 className="text-4xl font-extrabold text-slate-900 mb-4 leading-tight">{recipe.title}</h1>
            <p className="text-slate-600 text-lg mb-6 leading-relaxed">{recipe.description}</p>
            
            {(recipe.sourceUrl || manualSourceUrl) && (
                 <a href={manualSourceUrl || recipe.sourceUrl} target="_blank" rel="noreferrer" className="inline-flex items-center gap-1 text-sm text-blue-600 hover:underline mb-4">
            ...
            <div className="grid grid-cols-4 gap-2 mt-6">
                ...
                <div className="bg-slate-50 p-3 rounded-lg text-center">
                    ...
                    <div className="font-bold text-slate-700">{recipe.nutrition.protein}</div>
                </div>
                 <div className="bg-slate-50 p-3 rounded-lg text-center">
                    <div className="text-xs text-slate-400 uppercase font-bold">Carb</div>
                    <div className="font-bold text-slate-700">{recipe.nutrition.carbs}</div>
                </div>
                 <div className="bg-slate-50 p-3 rounded-lg text-center">
                    <div className="text-xs text-slate-400 uppercase font-bold">Fat</div>
                    <div className="font-bold text-slate-700">{recipe.nutrition.fat}</div>
                </div>
            </div>

            <button 
                onClick={() => { setCookingStep(0); setCookingIngredientsChecklist({}); setAppState(AppState.COOKING_MODE); }}
          ...
          <div className="relative aspect-video bg-slate-200 rounded-2xl overflow-hidden shadow-inner">
            {generatedImage ? (
              <img src={generatedImage} alt={recipe.title} className="w-full h-full object-cover animate-fadeIn" />
            ) : (
            ...
            <div className="lg:col-span-2">
                 <div className="bg-white p-8 rounded-2xl shadow-sm border border-slate-100">
                    <div className="flex items-center justify-between mb-6">
                        ...
                        <button 
                            onClick={() => handlePlayAudio(recipe.instructions.join('. '))}
  ...
  const renderCookingMode = () => {
      if (!recipe) return null;
      const currentInstruction = recipe.instructions[cookingStep];
      const detectableTime = !timer?.isActive ? parseDuration(currentInstruction) : null;
          ...
          <div className="fixed inset-0 bg-white z-50 flex flex-col">
              <div className="h-16 border-b border-slate-100 flex items-center justify-between px-6 bg-white">
                  <div className="flex items-center gap-4">
                      ...
                      <span className="font-bold text-slate-900 truncate max-w-[200px] sm:max-w-md">{recipe.title}</span>
                  </div>
              ...
              <div className="flex-1 flex overflow-hidden">
                  <div className="flex-1 flex flex-col relative overflow-y-auto">
                      <div className="h-1 bg-slate-100 w-full">
                          <div 
                              ...
                              style={{ width: `${((cookingStep + 1) / recipe.instructions.length) * 100}%` }}
                          />
                      </div>
                      ...
                      <div className="flex-1 flex flex-col items-center justify-center p-8 max-w-4xl mx-auto w-full text-center">
                          <div className="text-chef-600 font-bold uppercase tracking-widest mb-4 text-sm">
                              Step {cookingStep + 1} of {recipe.instructions.length}
                          </div>
                          ...
                          {timer?.isActive && (
                              <div className="mb-8 inline-flex flex-col items-center justify-center w-48 h-48 rounded-full border-4 border-chef-500 bg-chef-50 text-chef-800 shadow-lg animate-pulse-slow">
                                  <span className="text-5xl font-mono font-bold">{formatTime(timer.secondsLeft)}</span>
                                  <span className="text-sm font-bold uppercase mt-1">Timer</span>
                                  <button 
                          ...
                          {!timer?.isActive && detectableTime && (
                               ...
                               >
                                   ...
                                   Start Timer ({Math.floor(detectableTime / 60)}m)
                               </button>
                          )}
                      </div>
                      ...
                      <div className="p-6 border-t border-slate-100 flex justify-between items-center bg-white">
                          ...
                          <button 
                              ...
                              disabled={cookingStep === recipe.instructions.length - 1}
                  ...
                  {showCookingIngredients && (
                      <div className="w-80 bg-slate-50 border-l border-slate-200 overflow-y-auto p-6 animate-slideLeft">
                          <h3 className="font-bold text-slate-900 mb-4 text-lg">Ingredients</h3>
